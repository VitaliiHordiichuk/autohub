...
export async function setProductImageDisplayMode(req,res) { try { return res.json({success:true,images:await ProductImageService.setDisplayMode(req.params.productId,req.params.imageId,req.body?.mode)}); } catch(error){ return fail(res,error); } }
export async function reprocessProductImage(req,res) { try { return res.status(202).json({success:true,images:await ProductImageService.reprocess(req.params.productId,req.params.imageId)}); } catch(error){ return fail(res,error); } }
